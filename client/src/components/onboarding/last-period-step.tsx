...
export function LastPeriodStep({ initialValue, onNext, onBack }: LastPeriodStepProps) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center">
      <h2 className="font-heading font-bold text-2xl mb-6">{t('lastPeriodStep.title')}</h2>
      <p className="text-neutral-800 mb-6 max-w-xs">{t('lastPeriodStep.subtitle')}</p>
        ...
        <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-4 mb-6">
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {Array.from({ length: prevMonthDays }).map((_, index) => (
              <div key={`prev-${index}`} className="calendar-day text-neutral-400"></div>
            ))}
