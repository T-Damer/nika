        ...
        <Card className="mb-6 overflow-hidden">
          ...
          <CardContent className="pb-5">
            ...
            <div className="mb-5">
              <div className="relative h-8 bg-neutral-100 dark:bg-neutral-800 rounded-full overflow-hidden">
                {phases.map((phase, index) => {
                  ...
                  return (
                    <div 
                      key={phase.name}
                      className={`absolute top-0 bottom-0 ${phase.className} ${currentPhase.name === phase.name ? 'opacity-100' : 'opacity-50'}`}
                      style={{ 
                        left: `${phaseStart}%`, 
                        width: `${phaseWidth}%` 
                      }}
              ...
              <div className="flex justify-between text-xs mt-1">
                {phases.map(phase => (
                  <div 
                    key={phase.name}
                    className={`text-center ${currentPhase.name === phase.name ? 'font-bold' : ''}`}
                  >
            ...
            <div className="flex items-center">
              <div className={`p-2 rounded-full ${phases.find(p => p.name === currentPhase.name)?.className} mr-3`}>
              ...
              <div>
                ...
                <p className="text-xs text-neutral-500 dark:text-neutral-500 mt-1">
                  {t('insights.dayOfCycle', { day: currentCycleDay, total: user.cycleLength })}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col items-center p-3 bg-neutral-50 dark:bg-neutral-800 rounded-lg">
                ...
                <p className="text-sm text-neutral-600 dark:text-neutral-400">{t('insights.avgCycleLength')}</p>
              </div>
              <div className="flex flex-col items-center p-3 bg-neutral-50 dark:bg-neutral-800 rounded-lg">
                <Droplets className="h-6 w-6 text-primary mb-2" />
                <h3 className="font-medium">{user.periodLength}</h3>
                <p className="text-sm text-neutral-600 dark:text-neutral-400">{t('insights.avgPeriodLength')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
